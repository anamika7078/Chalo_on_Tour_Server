...
function formatMultilineHtml(text) {
  ...
  return esc(String(text).trim()).replace(/\r?\n/g, '<br />');
}

function getTripImages(lead) {
  const defaults = [
    "https://images.unsplash.com/photo-1594911772125-07fc7a2d8d9f?q=80&w=2070&auto=format&fit=crop",
    "https://images.unsplash.com/photo-1626621341517-bbf3d9990a23?q=80&w=1974&auto=format&fit=crop",
    "https://images.unsplash.com/photo-1621840212003-7f287e0767ce?q=80&w=2070&auto=format&fit=crop",
  ];
  const custom = Array.isArray(lead.tripImages) ? lead.tripImages.filter(Boolean) : [];
...
function buildTourSummaryHtml(lead) {
  ...
  const paxBreakupSummary = Array.isArray(lead.paxBreakup) && lead.paxBreakup.length
    ? lead.paxBreakup
        ...
        .filter(Boolean)
        .join(', ')
    : [lead.paxCount, lead.paxType].filter(Boolean).join(' ') || '–';
  const tripImages = getTripImages(lead);

  const summaryRows = [
    ['01. Total Package Cost', formatAmount(lead.total_amount)],
    ['02. Package Cost Per Person', formatAmount(lead.packageCostPerPerson)],
    ['03. Total No. of Pax', lead.paxCount != null ? String(lead.paxCount) : '–'],
    ['04. Pax Type Breakdown', paxBreakupSummary],
    ['05. Vehicle Type', lead.vehicleType || '–'],
    ['06. Hotel Category', lead.hotelCategory || '–'],
    ['07. Meal Plan', lead.mealPlan || '–'],
    ['08. Tour Duration', [lead.tourNights != null && `${lead.tourNights} Nights`, lead.tourDays != null && `${lead.tourDays} Days`].filter(Boolean).join(' / ') || '–'],
    ['09. Tour Date', lead.tourStartDate && lead.tourEndDate ? `${formatDate(lead.tourStartDate)} to ${formatDate(lead.tourEndDate)}` : formatDate(lead.travel_date)],
    ['10. Pick up', lead.pickupPoint || '–'],
    ['11. Drop', lead.dropPoint || '–'],
    ['12. Destinations', Array.isArray(lead.destinations) && lead.destinations.length ? esc(lead.destinations.join(', ')) : (lead.destination || '–')],
  ];
  ...
  <div class="page">
    ...
    <div class="page-body">
      ...
      <div class="image-grid">
        <div class="main-img-box"><img src="${tripImages[0]}" class="tour-img" /></div>
        <div class="sub-imgs-box">
          <div class="sub-img-box"><img src="${tripImages[1]}" class="tour-img" /></div>
          <div class="sub-img-box"><img src="${tripImages[2]}" class="tour-img" /></div>
        </div>
      </div>
          ...
          <tbody>
            ${flightRows.map((f, i) => `
              <tr>
                ...
                <td style="text-align:center">${f.fare != null ? esc(Number(f.fare).toLocaleString('en-IN')) : '–'}</td>
              </tr>
            `).join('')}
          </tbody>
        </table>
      ...
      <div class="footer">
        ${lead.termsAndConditions?.trim() ? `
          <div style="margin-bottom:24px;">
            <div class="heading-box blue-bg" style="font-size:13pt; padding:8px 20px;">Terms And Conditions</div>
            <p class="itinerary-desc" style="margin-top:12px;">${formatMultilineHtml(lead.termsAndConditions)}</p>
          </div>
        ` : ''}
        ${lead.memorableTrip?.trim() ? `
          <div style="margin-bottom:24px;">
            <div class="heading-box red-bg" style="font-size:13pt; padding:8px 20px;">Memorable Trip</div>
            <p class="itinerary-desc" style="margin-top:12px;">${formatMultilineHtml(lead.memorableTrip)}</p>
          </div>
        ` : ''}
        <div class="closing-msg">
          ${esc(lead.memorableTrip || "It's time to say goodbye - Phir Milenge!!!")}
        </div>
