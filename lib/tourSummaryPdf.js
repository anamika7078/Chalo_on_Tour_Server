...
/** Default hero image URLs (slot 1–6). Admin can override via lead.tripImages / lead.heroImageUrls. */
...
function getHeroImageUrls(lead) {
  const customImages = Array.isArray(lead.tripImages) && lead.tripImages.length
    ? lead.tripImages
    : Array.isArray(lead.heroImageUrls) && lead.heroImageUrls.length
      ? lead.heroImageUrls
      : [];
  return [...customImages.filter(Boolean), ...DEFAULT_HERO_IMAGE_URLS].slice(0, 6);
}

function decodeDataUrlBuffer(url) {
...
/** Fetch image from URL or data URL; returns Buffer or null. */
async function fetchImageBuffer(url, timeoutMs = 8000) {
  if (!url || typeof url !== 'string') return null;
  const dataUrlBuffer = decodeDataUrlBuffer(url);
...
function drawFlightTable(doc, flights) {
  const cols = [30, 70, 70, 115, 95, 75];
  const headers = ['Sr', 'From', 'To', 'Airline Info', 'PNR / Booking', 'Fare (Rs)'];
  const rows = (Array.isArray(flights) ? flights : []).map((f, i) => [
    ...
    val(f.airline),
    val(f.pnr),
    f.fare != null ? Number(f.fare).toLocaleString('en-IN') : '–',
  ]);
  if (rows.length === 0) {
...
async function buildTourSummaryPdf(lead, res) {
  ...
  const summaryRows = [
    ...
    ['Package Cost Per Person', formatAmount(lead.packageCostPerPerson)],
    ['Total No. of Pax', lead.paxCount != null ? String(lead.paxCount) : '–'],
    ['Pax Type Breakdown', paxBreakupSummary],
